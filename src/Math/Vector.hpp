...
public:
	...
	Vector3f isNormalized(float errorMargin = 1.e-4f) const;
...
FORCEINLINE Vector3f::Vector3f(const Vector& vecIn) : vec(vecIn)
{}

FORCEINLINE Vector3f::Vector3f() : vec(VectorConstants::ZERO)
{}

FORCEINLINE Vector3f::Vector3f(float val) : vec(Vector::make(val, val, val, 0.0f))
{}

FORCEINLINE Vector3f::Vector3f(float xIn, float yIn, float zIn) : vec(Vector::make(xIn, yIn, zIn, 0.0f))
{}
