...
public:
	...
	static FORCEINLINE float matrixDeterminant4x4(float* outSin, float* outCos, const void* mat)
	{
		float sVals[6];
		float cVals[6];

		float* s = outSin == nullptr ? sVals : outSin;
		float* c = outCos == nullptr ? cVals : outCos;
	...
	FORCEINLINE void storeAlignedStream(float* result) const
	{
	...
	{
		static const SSEVector sign_mask(SSEVector::make((uint32)0x7FFFFFFF, 0x7FFFFFFF, 0x7FFFFFFF, 0x7FFFFFFF));
		SSEVector vec;
	...
	{
		static const SSEVector sign_mask(SSEVector::make((uint32)0x80000000, 0x80000000, 0x80000000, 0x80000000));
		SSEVector vec;
		vec.data = _mm_and_ps(data, sign_mask.data);
		return vec;
	}
	...
	FORCEINLINE SSEVector cross3(const SSEVector& other) const
	{
		...
		const __m128 t1 = _mm_shuffle_ps(other.data, other.data, SSEVector_SHUFFLEMASK(2, 0, 1, 3));
		const __m128 t3 = _mm_shuffle_ps(data, data, SSEVector_SHUFFLEMASK(2, 0, 1, 3));
		const __m128 t2 = _mm_shuffle_ps(other.data, other.data, SSEVector_SHUFFLEMASK(1, 2, 0, 3));
		SSEVector vec;
	...
	{
		SSEVector vec;
		vec.data = _mm_xor_ps(other.data, _mm_and_ps(mask.data, _mm_xor_ps(data, other.data)));
		return vec;
	}
};